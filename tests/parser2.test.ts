...
let parser2Input = [
  ...
  ['?=one:two\n' + 'test=Blah `%0.r(o,huzzah)` `%1.t(w)`', '{"test":"Blah huzzahne t"}'],
  ...
  ['?=[one;two]\n' + 'test=Blah `%0.s` %1.s', '{"test":"Blah One Two"}'],
  [
    '_person(  \n' +
      '  name(\n' +
      '    first=John\n' +
  ...
  ['_branch="alex.";_root=d;namespace=`%branch`blah.`%root`', '{"namespace":"alex.blah.d"}'],
  ['namespace=`%branch`blah.`%root`', '{"namespace":"%branchblah.%root"}'],
  [
    '\n' +
      '_test=abc\n' +
      '\n' +
