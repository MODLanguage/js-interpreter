...
var MODLLexer_1 = require("./generated/MODLLexer");
var MODLParser_1 = require("./generated/MODLParser");
var ModlObject_1 = require("./ModlObject");
var NUMBER_REGEXP = new RegExp('^[0-9]*$');
var REFERENCE_REGEXP = new RegExp('%[\\w\\[\\].,%()]*');
var GRAVE_REGEXP = new RegExp('`[\\w\\[\\],.%()]*`');
var MODLListener = /** @class */ (function () {
    function MODLListener(modlObject, loadedClasses, keys, objectIndex, modlState) {
        if (modlState === void 0) { modlState = null; }
        this.modlObject = modlObject;
        this.loadedClasses = loadedClasses;
        this.keys = keys;
        this.objectIndex = objectIndex;
        this.modlState = modlState;
        this.modlState = this.modlState || {
            keys: {},
            objectIndex: [],
            loadedClasses: {}
        };
    }
    MODLListener.parse = function (input, modlObject, loadedClassesIn, keysIn, objectIndexIn, modlState) {
        if (loadedClassesIn === void 0) { loadedClassesIn = null; }
        if (keysIn === void 0) { keysIn = null; }
        if (objectIndexIn === void 0) { objectIndexIn = null; }
        if (modlState === void 0) { modlState = null; }
        if (modlState === null) {
            modlState = {
                keys: {},
        ...
        if (objectIndexIn === null) {
            modlState.objectIndex = [];
        }
        else {
            modlState.objectIndex = objectIndexIn;
        }
        if (keysIn == null) {
            modlState.keys = {};
        }
        else {
            modlState.keys = keysIn;
        }
        if (loadedClassesIn == null) {
            modlState.loadedClasses = {};
            MODLListener.initialiseLoadedClasses(modlState.loadedClasses);
        }
        else {
            modlState.loadedClasses = loadedClassesIn;
        }
        // this.modlObject = modlObject;
        // keys = {};
        var chars = new antlr4ts_1.ANTLRInputStream(input);
        var lexer = new MODLLexer_1.MODLLexer(chars);
        var tokens = new antlr4ts_1.CommonTokenStream(lexer);
        var parser = new MODLParser_1.MODLParser(tokens);
        var tree = parser.modl();
        var modl = new MODLListener(modlObject, modlState.loadedClasses, modlState.keys, modlState.objectIndex, modlState);
        tree_1.ParseTreeWalker.DEFAULT.walk(modl, tree);
        if (modlObject[0] instanceof ModlObject_1.ModlArray) {
            console.log('ARRAY!');
        }
        if (modlObject[0] instanceof ModlObject_1.ModlMap) {
            console.log('MAP!');
        }
        if (modlObject[0] instanceof ModlObject_1.ModlPair) {
            console.log('PAIR!');
        }
        return modlObject;
    };
    MODLListener.prototype.enterEveryRule = function (ctx) {
        console.log('ENTER EVERY RULE');
    };
...
ModlListener.parse = function(
  ...
  ParseTreeWalker.DEFAULT.walk(modl, tree);
...
ModlListener.prototype.processTop_level_conditional = function(ctx) {
  ...
  for (i = 0; i < ctx.condition_test().length; i++) {
    ...
    if (this.evaluates(ctx.condition_test(i))) {
      var conditionalReturn = this.processTop_level_conditional_return(
...
ModlListener.prototype.processValue_conditional = function(ctx) {
  ...
  if (ctx.value_conditional_return().length > ctx.condition_test().length) {
    // var conditionTest = new ConditionTest();
    var conditionalReturn = this.processValue_conditional_return(
...
ModlListener.prototype.processArray_conditional = function(ctx) {
  ...
  if (ctx.array_conditional_return().length > ctx.condition_test().length) {
    // var conditionTest = new ConditionTest();
    var conditionalReturn = this.processArray_conditional_return(
...
ModlListener.prototype.processMap_conditional = function(ctx) {
  ...
  if (ctx.map_conditional_return().length > ctx.condition_test().length) {
    // var conditionTest = new ConditionTest();
    var conditionalReturn = this.processMap_conditional_return(
...
*/
var INITCAPS = function (toTransform) {
    return toTransform.replace(/(?:^|\s)\S/g, function (a) {
        return a.toUpperCase();
    });
};
var REPLACE = function (toTransform, parameter1, parameter2) {
    return toTransform.replace(parameter1, parameter2);
};
var TRIM = function (toTransform, parameter1) {
    var index = toTransform.indexOf(parameter1);
    if (index < 0) {
